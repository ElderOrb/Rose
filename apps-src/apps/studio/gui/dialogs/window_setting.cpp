...
void twindow_setting::append_float_widget(twindow& window)
{
	...
	gui2::tedit_box::tparam param(_("New float widget"), null_str, _("ID"), default_float_widget_id);
	{
		param.text_changed = boost::bind(&mkwin_controller::verify_new_float_widget, &controller_, _1, nposm);
...
{
	int res = gui2::show_message("", _("Do you want to erase this float widget?"), gui2::tmessage::yes_no_buttons);
	if (res != gui2::twindow::OK) {
		return;
	}
