...
std::string mkwin_controller::gui_tpl_widget_path(const std::string& app)
{
	...
	if (app != "rose") {
		return game_config::path + "/data/gui/" + game_config::generate_app_dir(app) + "/tpl_widget";
	} else {
...
{
	if (!enumerate_child(boost::bind(&mkwin_controller::did_no_multiline_widget, this, _1, _2), top_units, false)) {
		return false;
	}
	return true;
}
...
bool mkwin_controller::did_valid_id(const unit* u, bool show_error)
{
	...
	if (top_.window->cell().window.scene && id == main_map_widget_id) {
		...
		if (u->cell().widget.width_is_max && !u->cell().widget.height.empty()) {
...
bool mkwin_controller::enumerate_child2(const boost::function<bool (const unit*, bool)>& did, const unit::tchild& child, bool show_error) const
{
...
bool mkwin_controller::enumerate_child(const boost::function<bool (const unit*, bool)>& did, const std::vector<unit*>& top_units, bool show_error) const
{
...
bool mkwin_controller::window_is_valid(bool show_error)
{
	...
	{
		if (!enumerate_child(boost::bind(&mkwin_controller::did_valid_id, this, _1, _2), top_units, show_error)) {
			return false;
		}
	}
	return true;
...
void mkwin_controller::quit_confirm(int mode, bool dirty)
{
	...
	const int res = gui2::show_message(_("Quit"), message, gui2::tmessage::yes_no_buttons);
...
std::string calculate_app_from_gui_cfg_file(const std::string& file)
{
	std::string app;
	if (!file.empty()) {
		const std::string prefix = game_config::path + "/data/gui/";
...
void mkwin_controller::longpress_widget(bool& halt, const tpoint& coordinate, gui2::twindow& window, const std::string& type, const std::string& definition)
{
	...
	window.start_drag(coordinate, boost::bind(&mkwin_controller::did_drag_mouse_motion, this, _1, _2, boost::ref(window)),
		boost::bind(&mkwin_controller::did_drag_mouse_leave, this, _1, _2, _3));
}
