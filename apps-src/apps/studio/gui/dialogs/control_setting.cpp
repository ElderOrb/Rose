...
void tcontrol_setting::pre_advanced(twindow& window)
{
	...
	if (!u_.is_text_box()) {
		...
		ttoggle_button* toggle = find_widget<ttoggle_button>(advanced_page, "text_box_password", false, true);
	...
	if (!u_.is_report()) {
		...
		ttext_box* text_box = find_widget<ttext_box>(advanced_page, "report_fixed_cols", false, true);
